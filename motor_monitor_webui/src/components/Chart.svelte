<script lang="ts">
  ...
  function truncateDatasets(labels: any[], datasets: any[], maxLength: number) {
  ...
  $effect(() => {
    if (chart) {
      ...
      // 保留图表的隐藏状态
      const visibility = chart.data.datasets.map((dataset) => dataset.hidden);
      // 更新数据
  ...
  export function addNewData(newData: any) {
    if (chart) {
      chart.data.labels?.push(newData.label);
      chart.data.datasets.forEach((dataset, index) => {
        dataset.data.push(newData.datasets[index]);
      });
      chart.update();
    }
  }
